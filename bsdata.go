...
func GetBSData(repo, tag string) ([]*Catalogue, error) {
	...
	err = cleanUp(repo)
	if err != nil {
		return nil, ErrBSDataCouldNotCleanUpTempDirs
	}

	// clone the repo
	err = clone(repo, tag)
	if err != nil {
		return nil, ErrBSDataCouldNotCloneRepo
	}

	// get the cat files
	files, err := getCatFiles(repo)
	if err != nil {
		return nil, ErrBSDataCouldNotReadRepoFiles
	}
	...
	for _, file := range files {
		...
		if err != nil {
			return nil, ErrBSDataCouldNotReadRepoFile
		}

		var cat Catalogue
		err = xml.Unmarshal(b, &cat)
		if err != nil {
			return nil, ErrBSDataCouldNotUnmarshalCatalogueXML
		}
